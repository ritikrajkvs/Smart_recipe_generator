      ...
      <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-8 mb-8 relative overflow-hidden">
        ...
        <div className="grid grid-cols-3 gap-4 border-t border-slate-100 pt-6 mt-6">
          ...
          <div className="text-center">
            <p className="text-slate-400 text-xs font-bold uppercase">Calories (Total)</p>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100">
            <div className="flex justify-between items-center border-b pb-4 mb-4">
               ...
               <div className="flex items-center bg-slate-100 rounded-full p-1 shadow-inner">
                ...
                <button onClick={increaseServings} className="w-8 h-8 flex items-center justify-center bg-white rounded-full shadow-md hover:text-green-600 font-bold">+</button>
               </div>
            </div>

            <ul className="space-y-2">
              {scaledIngredients.map((ing, idx) => (
                <li key={idx} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 hover:bg-slate-100 transition-colors">
                  <span className="text-slate-700 capitalize font-medium">{ing.name}</span>
                  <span className="text-slate-600 text-sm font-semibold">{ing.qty} {ing.unit}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100">
            ...
            <div className="space-y-6">
              {recipe.steps.map((step, idx) => (
                <div key={idx} className="flex gap-4 p-4 bg-slate-50 rounded-xl shadow-sm hover:shadow-md transition-shadow group">
                  <div className="flex-shrink-0 w-10 h-10 bg-green-500 text-white rounded-full flex items-center justify-center font-bold text-xl ring-2 ring-green-300 group-hover:bg-green-600 transition-colors">
                    {idx + 1}
                  </div>
                  <div className="pt-1">
