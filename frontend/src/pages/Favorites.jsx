...
const Favorites = ()=>{
  ...
  useEffect(()=>{ const loadFavorites = async ()=>{ setLoading(true); try{ const results=[]; for(const id of favorites){ const res = await fetch(`http://localhost:5000/api/recipes/${id}`); const data = await res.json(); if(data.success) results.push({recipe: data.recipe, score:100, matchedIngredients:[], missingIngredients:[]}); } setFavRecipes(results);}catch(err){console.error(err);} setLoading(false); }; loadFavorites(); },[favorites]);
