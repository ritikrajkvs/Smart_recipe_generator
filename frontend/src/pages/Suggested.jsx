...
const Suggested = ()=>{
  ...
  const loadRecipes = async ()=>{ const res = await fetch('http://localhost:5000/api/recipes?ingredients=tomato'); const data = await res.json(); if(data.success) setRecipes(data.results.map(r=>r.recipe)); };
