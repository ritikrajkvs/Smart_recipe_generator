...
const Navbar = () => {
  ...
  // Helper function to render the auth buttons when user is logged out
  const renderAuthButtons = () => {
    ...
    // Default view when logged out and not on auth pages
    return (
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-white/80 backdrop-blur-xl border-b border-slate-100 shadow-sm transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        {/* Logo and Essential Links (Always visible for structure) */}
        <div className="flex items-center gap-6">
            <Link to="/" className="flex items-center gap-3 group">
                <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center text-2xl shadow-inner transition-transform">🥗</div>
            ...
            {/* Main Navigation Pill (Only show when logged in) */}
            {user && (
              <div className="hidden md:flex items-center gap-1 bg-slate-100/50 p-1.5 rounded-full border border-slate-200/50">
                {['/', '/results', '/suggested'].map(path => (
                  <Link key={path} to={path} className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${isActive(path) ? 'bg-white text-green-600 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}>
        ...
        {/* Right Side: User Profile / Saved / Auth Buttons */}
        <div className="flex items-center gap-4">
          {user ? (
            <>
              {/* 1. Saved Button (Icon Only) */}
              <Link 
                to="/favorites" 
              ...
              >
                ...
                {/* Counter Badge */}
                {favorites.length > 0 && (
                  <span className="absolute top-0 right-0 bg-red-500 text-white text-[10px] font-extrabold w-5 h-5 flex items-center justify-center rounded-full border-2 border-white shadow-sm">
                    {favorites.length}
                  </span>
                )}
              </Link>

              {/* 2. User Profile Display and Logout */}
              <div className="flex items-center gap-3 pl-4 border-l border-slate-200">
                {/* User's Name */}
                <p className="text-base font-bold text-slate-700 hidden lg:block">{user.username}</p>
